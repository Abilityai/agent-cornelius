...
**MANDATORY: You MUST use Smart Connections MCP tools for ALL sampling and analysis**
...
1. **Start with random seed notes** using semantic search:
   ...
   - For each term, use `mcp__smart-connections__search_notes` with limit=10, threshold=0.5
...
2. **Sample from each starting cluster:**
   ...
   - Use `mcp__smart-connections__get_similar_notes` on each seed with threshold=0.5-0.7
   - This reveals the local neighborhood around each seed

3. **Select analysis targets:**
   - From the similar notes, pick 2-3 notes from DIFFERENT domains
...
**Example workflow:**
```
Search "dopamine" → Get seed note [[Dopamine drives motivation]]
Get similar notes → [[Uncertain cues give higher reward]] (0.68)

Search "reference points" → Get seed note [[Reference point dependence]]
Get similar notes → [[Loss aversion bias]] (0.72)
...
**CRITICAL: Use BOTH MCP tools AND analytical reasoning together**

**MANDATORY Process:**
1. **For each pair of notes from different domains:**

   a. **Use MCP tools to get actual similarity scores:**
      - Use `mcp__smart-connections__get_similar_notes` on Note A with Note B's path
   ...
   c. **Document both scores:**
      - Semantic similarity: [actual score from MCP]
      - Conceptual strength: [your 1-5 star rating]
      - **Low semantic (0.50-0.70) + high conceptual (4-5 stars) = JACKPOT**

2. **Use connection graphs for triangulation:**
   - For strong connections, use `mcp__smart-connections__get_connection_graph` on both notes
   - Look for third notes that connect to both (consilience zones)
   - Depth=2, max_per_level=5, threshold=0.6
